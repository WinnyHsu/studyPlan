...
export async function getDailyProgress(date: string): Promise<DailyProgress | null> {
  try {
    const res = await notion.databases.query({ database_id:DB.daily, filter:{ property:"date", date:{ equals:date } } });
    if (!res.results.length) return null;
    const p = res.results[0] as any;
    return {
      id:           p.id,
...
export async function getMonthProgress(year:number, month:number) {
  ...
  const res = await notion.databases.query({ database_id:DB.daily, filter:{ and:[
...
export async function addWrongQuestion(q: WrongQuestion) {
  await notion.pages.create({ parent:{ database_id:DB.wrong }, properties:{
