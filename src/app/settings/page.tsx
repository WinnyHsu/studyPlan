...
function SettingsPage() {
  ...
  if(loading) return <LoadingScreen message="載入設定資料..."/>;
    ...
    <div className="page-pad">
      ...
      <Card style={{marginBottom:12}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {[
            ["讀書計畫開始日","startDate","date"],
            ["考試日期","examDate","date"],
            ["考試名稱","examName","text"],
            ["每日目標（分鐘）","dailyGoalMinutes","number"],
            ["你的名字（選填）","userName","text"],
          ].map(([label,key,type])=>(
            <div key={key as string}>
              <label style={{fontSize:12,color:"var(--color-text-muted)",display:"block",marginBottom:4}}>{label}</label>
              <input type={type as string} value={String(config[key as keyof AppConfig])}
                onChange={e=>setConfig(c=>({...c,[key]:type==="number"?Number(e.target.value):e.target.value}))}
                style={inputStyle} placeholder={label as string}/>
            </div>
          ))}
        </div>
      </Card>
