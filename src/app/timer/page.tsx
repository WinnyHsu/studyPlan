...
function TimerPage() {
  ...
  const saveTime=async()=>{
    ...
    if(mins<1) return;
    await fetch("/api/schedule",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({date:today,addMinutes:mins})});
    setSaved(true);
  };
    ...
    <div className="page-pad">
      ...
      <Card style={{textAlign:"center",padding:"28px 16px",marginBottom:14}}>
        ...
        <div style={{display:"flex",gap:8,justifyContent:"center",flexWrap:"wrap"}}>
          <button onClick={start} disabled={running} style={{padding:"10px 20px",background:"var(--color-blue)",color:"white",border:"none",borderRadius:8,fontSize:14,cursor:"pointer",opacity:running?.5:1}}>▶ 開始</button>
          <button onClick={pause} disabled={!running} style={{padding:"10px 20px",background:"var(--color-surface)",border:"1px solid var(--color-border)",borderRadius:8,fontSize:14,cursor:"pointer",opacity:!running?.5:1}}>⏸ 暫停</button>
          <button onClick={reset} style={{padding:"10px 20px",background:"var(--color-surface)",border:"1px solid var(--color-border)",borderRadius:8,fontSize:14,cursor:"pointer"}}>↺ 重置</button>
          {secs>60&&!saved&&(
            <button onClick={saveTime} style={{padding:"10px 20px",background:"#EAF3DE",color:"#27500A",border:"1px solid #97C459",borderRadius:8,fontSize:14,cursor:"pointer"}}>💾 儲存</button>
          )}
      ...
      <Card>
        <div style={{fontSize:13,fontWeight:500,marginBottom:8}}>💡 每日時間分配</div>
        <div style={{fontSize:12.5,color:"var(--color-text-muted)",lineHeight:1.8}}>
          週一~五（2小時）：<br/>
          ・00:10 基礎心算暖身<br/>
          ・01:10 數學進度（公式+手算）<br/>
          ・01:20 物理進度（定義+受力圖）<br/>
          ・02:00 錯題檢討 + 航空英文<br/><br/>
          週六日（3小時）：<br/>
          ・綜合大演練、模擬測驗、錯題本複習
        </div>
      </Card>
