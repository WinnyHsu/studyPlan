...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  // SSR: render desktop layout until mounted
  if(!mounted) return (
  ...
  if(isMobile) return (
    <div style={{minHeight:"100dvh",display:"flex",flexDirection:"column",background:"var(--color-bg)"}}>
      ...
      <main className="scroll-hide" style={{flex:1,overflow:"auto"}}>{children}</main>
      <BottomNav/>
    </div>
  );
  ...
  return (
    <div style={{display:"flex",minHeight:"100vh"}}>
      <Sidebar daysLeft={days} weekNumber={week} stage={stage.label}/>
      <main style={{flex:1,display:"flex",flexDirection:"column",overflow:"auto"}}>{children}</main>
    </div>
  );
}
