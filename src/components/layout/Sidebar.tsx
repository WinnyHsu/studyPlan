...
export default function Sidebar({ daysLeft, weekNumber, stage }: Props) {
  ...
  return (
    <aside style={{ width:210, flexShrink:0, background:"var(--color-surface)", borderRight:"1px solid var(--color-border)", display:"flex", flexDirection:"column", height:"100vh", position:"sticky", top:0 }}>
      <div style={{ padding:"20px 16px 14px", borderBottom:"1px solid var(--color-border)" }}>
        <div style={{ background:"var(--color-blue-light)", color:"var(--color-blue-text)", fontSize:11, fontWeight:500, padding:"3px 8px", borderRadius:4, display:"inline-block", marginBottom:8 }}>✈ PPL Prep</div>
      ...
      <nav style={{ padding:"10px 8px", flex:1 }}>
        {NAV.map(n => (
          <Link key={n.href} href={n.href} style={{ display:"flex", alignItems:"center", gap:9, padding:"8px 10px", borderRadius:8, marginBottom:2, fontSize:13, fontWeight:path===n.href?500:400, color:path===n.href?"var(--color-blue-text)":"var(--color-text-muted)", background:path===n.href?"var(--color-blue-light)":"transparent", textDecoration:"none", transition:"background 0.1s" }}>
      ...
      <div style={{ padding:"12px", borderTop:"1px solid var(--color-border)" }}>
        <div style={{ marginBottom:8, display:"flex", gap:6, flexWrap:"wrap" }}>
          ...
          <span style={{ background:"var(--color-amber-light)", color:"var(--color-amber-text)", fontSize:11, padding:"2px 8px", borderRadius:20, fontWeight:500 }}>
            {stage.includes("一")?"第一":stage.includes("二")?"第二":"第三"}階段
          </span>
        </div>
